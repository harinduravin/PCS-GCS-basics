...
class Sampling_sym_sub_constellation(Layer):
    def __init__(self, N, parity_k, tau, device, constellation, use_uniform):
        super().__init__()
...
class EndToEndSystem_bitwise_PAS(Model):
    def __init__(self, N, parity_k, constellation, device, constellation_orig, use_upsampling_filtering, use_uniform, tau=1.0):
        super().__init__()
        print('device', device)

        # self.sampling = SamplingMechanism_P0_trainable_parameter(N, tau, device, constellation)
        self.sampling = Sampling_sym_sub_constellation(N, parity_k, tau, device, constellation, use_uniform)
    ...
    def call(self, ebno_db, No):
        ...
        if self.use_upsampling_filtering:
            ...
            # norm_diff = tf.norm(y - x_mod, ord='euclidean')

            ## Test whether txfilter input and rxfilter output are same when channel is removed.
            ## Implement PAPR with filtering and sampling and test...
        else:
